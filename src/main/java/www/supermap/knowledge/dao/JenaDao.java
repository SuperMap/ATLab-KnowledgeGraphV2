...
public interface JenaDao {
	...
	/**
	 * 按照数据集名称查询数据集id， 注意：本方法不会验证数据集所属的数据源是否存在，因为主程序中先处理了数据源，之后才处理数据集
	 * 
	 * @param targetKnowledgeGraph
	 * @param dataSourceId
	 * @param dataSetName
	 ...
	 * @return
	 */
	public Map<String, List<String>> selectInfoByType(KnowledgeGraph targetKnowledgeGraph, List<Long> coverCellIds,
			String[] arType);
	...
	/**
	 * 不能直接通过model添加，因为model有作用域，保存时候已经出了作用域了，只好直接将grid先转成model，再存入图谱，这两个过程放在一个会话里完成
	 * @param targetKnowledgeGraph
	 * @param grid
	 */
