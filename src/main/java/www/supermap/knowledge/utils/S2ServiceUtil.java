...
public class S2ServiceUtil {
	 ...
	 * @return
	 */
	public static ArrayList<Long> getGeometryCoveringCells(Geometry geometry, int gridLevel) {
		if (geometry instanceof GeoPoint) {
			return getGeoPointCoveringCell((GeoPoint) geometry, gridLevel);
		} else if (geometry instanceof GeoLine) {
			return getGeoLineCoveringCells((GeoLine) geometry, gridLevel);
		} else if (geometry instanceof GeoRegion) {
			return getGeoRegionCoveringCells((GeoRegion) geometry, gridLevel);
		} else {
			return null;
		}
	}
	...
	public static ArrayList<Long> getGeoRegionCoveringCells(GeoRegion region, int gridLevel) {
		...
		/**
		 * 找到S2point按逆时针存的集合 思路：先找到一个Z轴最大值的点，尽量保证是凸点，然后找到相邻的两个点，计算法向量，判断z轴的值。
	...
	/**
	 * 以输出cell的中心点经纬度为中心，获得指定半径氛围内的所有网格Id
	 * 
	 * @param cell
	 * @param iRadius
	 * @param gridLevel
	 * @return
	 */
	public static List<Long> getCoveringCellIdsFromCell(S2CellId cell, double iRadius, int gridLevel) {
		S2LatLng la = cell.toLatLng();
		S2Point point = la.toPoint();
		Double capHeight = 1 - Math.cos(iRadius / 12742000);
