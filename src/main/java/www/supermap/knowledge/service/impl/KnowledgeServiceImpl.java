...
import www.supermap.knowledge.dao.impl.DataSourceDaoImpl;
...
public class KnowledgeServiceImpl implements KnowledgeService {
	...
	@Override
	public Map<String, List<GeoInfo>> queryByType(KnowledgeGraph targetKnowledgeGraph, double dLongitude,
			double dLatitude, double iRadius, String[] arType) {
		// 1. 计算经纬度和半径所构成的面积
		// 2. 找出格子里符合条件的id
		...
		for (Entry<String, List<String>> entry : geoTypeAndIds.entrySet()) {
			...
			for (String wholeId : entry.getValue()) {
				...
				// 5. 完善geoInfo信息
				geoInfo.setId(wholeId);
				geoInfo.setEntityType(dataSetName);
				geoInfo.setDataSourcePath(dataSourceName);
	...
	public boolean addDataByType(KnowledgeGraph targetKnowledgeGraph, String dataFile, String[] arTypes) {
		...
		HashMap<String, ArrayList<String>> dataSourceAndDataSetNames = dataSourceDao.selectAllDataSourceByFile(dataFile);
		...
		for (Entry<String, ArrayList<String>> entry : dataSourceAndDataSetNames.entrySet()) {
			...
			for (String dataSetName : entry.getValue()) {
				...
				// 4. 打开数据源，设置myGeometry的geometry和primaryid
				List<MyGeometry> myGeometrys = dataSourceDao.selectDataByDataSetName(dataSourceName, dataSetName);

				// 5.
				// 完善myGeometry信息，从数据源中读取时，已经设置了type和geometry属性，此步骤对剩下的datasourceId,datasetId,type,gridIds进行设置
				for (MyGeometry myGeometry : myGeometrys) {
					...
					myGeometry.setType(dataSetName);
				...
				// 5. 生成model，并将其存入知识图谱
				for (Grid grid : grids) {
					jenaDao.insertGridToKnowledge(targetKnowledgeGraph, grid);
				}
			}
		}
		return true;
